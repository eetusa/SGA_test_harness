...
public class SGA_parallel {
    ...
    private final Chromosome_parallel[] pop;
  //  private final ThreadLocalRandom gen = new ThreadLocalRandom();

    // The goal of Chromosome delivered as a parameter.
    public SGA_parallel(int popSize, String goal ) {
        POPULATION_SIZE = popSize;
        CHROMOSOME_LENGTH = goal.length();
        pop = new Chromosome_parallel[POPULATION_SIZE];
        for ( int i = 0; i < POPULATION_SIZE; i++ )
            pop[i] = new Chromosome_parallel ( goal, CHROMOSOME_LENGTH );
    }
    ...
    public int[] crossover () {
       ...
       // int parent1 = distribution.get ( gen.nextInt( distribution.size()));
        int parent1 = distribution.get ( ThreadLocalRandom.current().nextInt(0, distribution.size()) );
        int parent2;
        do {
          //  parent2 = distribution.get ( gen.nextInt( distribution.size()));
            parent2 = distribution.get ( ThreadLocalRandom.current().nextInt(0, distribution.size()) );
        ...
        if (Math.random() < CROSSOVER_PROB) {
            ...
            if (IS_CROSSOVER_POINT_RANDOM){
             //   crossover = gen.nextInt(CHROMOSOME_LENGTH - 1) + 1;
                crossover = ThreadLocalRandom.current().nextInt(0, (CHROMOSOME_LENGTH-1)) + 1;
            }
    ...
    public void mutation ( int popMember, boolean full ) {
        if ( full ) {
            for ( int i = 0; i < CHROMOSOME_LENGTH; i++ ) {
                if ( ThreadLocalRandom.current().nextDouble() < MUTATION_PROB ) {
    ...
    public void killTheStupid ( int n ) throws CloneNotSupportedException {
        Arrays.sort ( pop );
        for ( int i = 0; i < n; i++ )
            pop[POPULATION_SIZE-1-i] = (Chromosome_parallel) pop[i].clone();
    }
